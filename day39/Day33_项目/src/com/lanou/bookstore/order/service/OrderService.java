...
public Order queryOrderByOid(String oid) throws SQLException {
    ...
    public String updateOrder(String oid) throws SQLException, StateErrorException, AddressNotChangeException {
        orderDao.updateAdminState(1,oid);
        int b = orderDao.queryAdminStateByOid(oid);
        if (b==0){
            throw new StateErrorException();
        }else {
    ...
    public String confirm(String oid) throws SQLException, StateErrorException, StateNotSendException {
        int i = orderDao.queryAdminStateByOid(oid);
        if (i==0){
            throw new StateNotSendException();
        }else {
    ...
    public int changeAdminState(String oid) throws SQLException {
        int state = orderDao.getState(oid);
        ...
        if (i==0){
            if (state==0 || state==1){
                ...
                System.out.println("订单已完成就不用发货了吧");
            }
            int i1 = orderDao.queryAdminStateByOid(oid);
            if (i1==1){
                return state;
            }
        }else {
            System.out.println("货物早已发出去了");
        }
