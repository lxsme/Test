...
public class OrderDao {
    ...
    public void updateAdminState(int state,String oid) throws SQLException {
        Connection conn = C3P0Util.getConnection();
        String sql = "update orders set adminState=? where oid=?";
        queryRunner.update(conn,sql,state,oid);
        C3P0Util.release(null,null,conn);
    }

    public int queryAdminStateByOid(String oid) throws SQLException {
        Connection conn = C3P0Util.getConnection();
        String sql = "select adminState from orders where oid=?";
        int adminState = queryRunner.query(conn, sql, new ScalarHandler<>(), oid);
        C3P0Util.release(null,null,conn);
        return adminState;
    }
